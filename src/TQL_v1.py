...
def save_q_table(q_table, filename=f'q_table_training_p{curr_test}_tql_v1.pkl'):
    full_path = os.path.join(os.getcwd(), filename)
    with open(full_path, 'wb') as f:
        pickle.dump(q_table, f)
    print(f"Q-table saved to {full_path}")

def load_q_table(filename=f'TQL_results_{train_numb}\Tql_v1\p{prev_test}\q_table_training_p{prev_test}_tql_v1.pkl'):
    full_path = os.path.join(os.getcwd(), filename)
...
def load_q_table2(filename=f'q_table_training_p{curr_test}_tql_v1.pkl'):
    full_path = os.path.join(os.getcwd(), filename)
    with open(full_path, 'rb') as f:
        q_table = pickle.load(f)
    print(f"Q-table loaded from {full_path}")
...
def train(env, pre_trained=pre_trained, save=save, num_episodes=num_episodes_train):
    ...
    plt.savefig(f'Explorations_p{curr_test}_tql_v1.png')
    ...
    plt.savefig(f'Exploitations_p{curr_test}_tql_v1.png')
...
def test(env, num_episodes=num_episodes_test):
    ...
    plt.savefig(f'Test_States_in_Q_p{curr_test}_tql_v1.png')
    ...
    plt.savefig(f'Test_states_not_in_Q_p{curr_test}_tql_v1.png')
    ...
    plt.savefig(f'Test_Steps_p{curr_test}_tql_v1.png')
...
if __name__ == '__main__':
    ...
    src = os.getcwd()
    dest = f'{src}\TQL_results_{train_numb}\Tql_v1\p{curr_test}'
